...
aws_iam_role = f"arn:aws:iam::{aws_acc}:role/hopeHope"
debug_mode = strtobool(args["debug_mode"])
...
test_version = 'coxa_test2'
bucket_source           = args["bucket_name"] #'gn-datalake-development-raw'
key_source              = args["key"] #f'files/{test_version}/{today}/*'
...
# Logs to show sys and file information
logger.info(f"TempDir: {glue_tmp_dir}")
...
# The best solution is create a new glue job and use a workflow (airflow, glue workflow) to start the jobs one after other. To save time I put all codes togheter.
